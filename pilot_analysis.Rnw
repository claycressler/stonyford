...
<<echo=FALSE, dev='png', res=500, fig.keep='high', fig.width=12, fig.height=4, fig.cap="(A) The rearing environment (lab versus wild) has a large effect on worm burden at 3- or 4-weeks post-infection. However, within treatments, there is considerable individual variation in worm burden. (B) Worm burden has a strong negative correlation with the fraction of CD4+ T-cells expressing Th2 cytokines, as expected. (C) Expression of Th2 cytokines by CD4+ T-cells is negatively correlated with expression of Th1 cytokines, further indicating that chronicity is determined by the immune-parasite battle over Th1-Th2 cytokine expression.">>=
